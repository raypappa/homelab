...
def make_groups(
    dependencies: dict[str, DependencyFile],
    group_count: int = 1,
    matched_deps: set[str] | None = None,
    results: dict[int, set[str]] | None = None,
):
    if not matched_deps:
        matched_deps = set()
    if not results:
...
def update_app_config(
    groups: dict[int, set[str]], dependencies: dict[str, DependencyFile]
) -> None:
    for group in groups:
...
def check_appset_sync_strategy(max_wave: int, appset_file_path: pathlib.Path):
    with appset_file_path.open("r") as fh:
        app_set = yaml.load(fh, Loader=yaml.SafeLoader)
    if (
        int(
            app_set["spec"]["strategy"]["rollingSync"]["steps"][-1]["matchExpressions"][
                -1
            ]["values"][-1]
        )
        < max_wave
        ...
        raise ValueError(
            f"max wave {max_wave} is missing from the rollingSync strategy matches"
        )
...
def main():
    ...
    # i'm lazy and since i delete the entries track, i just re-create the data
    dependencies = load_dependencies()
